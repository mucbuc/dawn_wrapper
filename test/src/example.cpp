...
int main()
{
    ...
    input.write(data.data(), data.size() * sizeof(uint32_t));
    
    buffer_wrapper output = plugin.make_buffer(unsigned(data.size() * sizeof(uint32_t)), BufferType::Storage, false );
    
    auto bindgroup = comp.make_bindgroup();
    bindgroup.addBuffer(1, input);
    bindgroup.addBuffer(2, output);
    ...
    mapped.write(data.data(), data.size() * sizeof(uint32_t));
    
    encoder.copy_buffer_to_buffer(output, mapped);
