...
S v_2(uint start_byte_offset) {
  ...
  mat3 v_5 = v_1((16u + start_byte_offset));
...
S[4] v_7(uint start_byte_offset) {
  S a[4] = S[4](S(0, mat3(vec3(0.0f), vec3(0.0f), vec3(0.0f)), 0), S(0, mat3(vec3(0.0f), vec3(0.0f), vec3(0.0f)), 0), S(0, mat3(vec3(0.0f), vec3(0.0f), vec3(0.0f)), 0), S(0, mat3(vec3(0.0f), vec3(0.0f), vec3(0.0f)), 0));
  {
    uint v_8 = 0u;
    v_8 = 0u;
    while(true) {
...
void f_inner(uint tint_local_index) {
  {
    ...
    while(true) {
      ...
      w[v_11] = S(0, mat3(vec3(0.0f), vec3(0.0f), vec3(0.0f)), 0);
      {
        v_10 = (v_11 + 1u);
      }
    }
